...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Flaircoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Flaircoin Signed Message:\n";
...
{
    // Flaircoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Flaircoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Flaircoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Flaircoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xc1, 0xc1, 0xc1, 0xc1 }; // Flaircoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FlaircoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FlaircoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FlaircoinMiner(CWallet *pwallet)
{
    printf("FlaircoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running FlaircoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FlaircoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FlaircoinMiner, pwallet));
}
