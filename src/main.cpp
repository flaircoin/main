...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Flappycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Flappycoin Signed Message:\n";
...
{
    // Flappycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Flappycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Flappycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xc1, 0xc1, 0xc1, 0xc1 }; // Flappycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FlappycoinMiner : ProcessBlock, block not accepted");
    }
